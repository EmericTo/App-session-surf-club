...
/**
 * Convert data to a ArrayBuffer backed view if it is a SharedArrayBuffer backed view.
 * @private
 */
function _convertToArrayBuffer (data) {
  // only in WASM this will be an ArrayBufferView backed by a SharedArrayBuffer, it cannot happen on local
  if (ArrayBuffer.isView(data) && data.buffer instanceof SharedArrayBuffer) {
    if (Buffer.isBuffer(data)) {
      // Buffer.prototype.slice() works differently compared to TypedArray.prototype.slice(), so we convert it first
    ...
    if (this._isStreamInput()) {
      ...
      this.on('finish', () => {
        ...
        sharp.pipeline(this.options, (err, data, info) => {
          callback(err, _convertToArrayBuffer(data), info);
        });
      });
    } else {
      ...
      this.once('finish', () => {
        ...
        sharp.pipeline(this.options, (err, data, info) => {
          if (err) {
            ...
            this.emit('info', info);
            this.push(_convertToArrayBuffer(data));
          }
          this.push(null);
        ...
        this.once('finish', () => {
          ...
          sharp.pipeline(this.options, (err, data, info) => {
            if (err) {
              ...
              data = _convertToArrayBuffer(data);
